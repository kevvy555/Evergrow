...
export class SparkSystem {
  constructor(world, mergeSystem, perkSystem, random = Math.random) { this.world = world; this.mergeSystem = mergeSystem; this.perkSystem = perkSystem; this.random = random; }
  ...
  maybeSpawn() {
    ...
    const emptyCells = [];
    for (let y = 0; y < this.world.rows; y += 1) for (let x = 0; x < this.world.columns; x += 1) if (!this.world.getCell(x, y)) emptyCells.push({ x, y });

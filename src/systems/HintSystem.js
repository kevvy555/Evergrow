...
export class HintSystem {
  ...
  getPrimedCellKeys() {
    ...
    for (const hint of this.getPrimedClusters()) {
      for (const cell of hint.cells) primed.add(cellKey(cell.x, cell.y));
    }
    return primed;
  }
  ...
  getPrimedClusters() {
    ...
    for (const entity of this.world.cells.values()) {
      ...
      if (cluster.length === GAME_CONFIG.mergeCount - 1) result.push({ level: entity.level, cells: cluster });
    }
    return result;
  }

  getBestPrimedCluster() {
    return this.getPrimedClusters().sort((a, b) => b.level - a.level)[0] ?? null;
  }
}

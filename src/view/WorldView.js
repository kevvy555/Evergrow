...
export class WorldView extends Container {
  ...
  renderWorld() {
    ...
    const backdrop = new Graphics()
      ...
      .fill({ color: bloomActive ? 0x253154 : 0x10243f, alpha: 0.94 })
  ...
  play(events) {
    ...
    for (const event of events) {
      ...
      if (event.type === 'radiantBorn') {
        this.#particleBurst(event.x, event.y, RADIANT, 16);
        this.#announcement('✦ RADIANT LIFE', `${ENTITY_DEFINITIONS[event.level].emoji} ${ENTITY_DEFINITIONS[event.level].name}`);
      }
      ...
      if (event.type === 'wonderDiscovered') {
        for (const cell of event.cells ?? []) this.#particleBurst(cell.x, cell.y, 0xc8a7ff, 10);
        this.#announcement(`WONDER DISCOVERED ${event.icon}`, event.name, `+${event.reward}`);
      }
      ...
      if (event.type === 'goalComplete') this.#announcement('GOAL COMPLETE!', event.label, `+${event.reward}`);
      ...
      if (event.type === 'bloomStart') this.#announcement('🌸 WORLD BLOOM!', `${event.turns} empowered taps`, 'Trees from every tap');
      if (event.type === 'bloomEnd') this.#floatingLabelFromScreen('🌸 Bloom rests…', 0xf2a7ff);
      if (event.type === 'evolutionChosen') this.#announcement(`EVOLVED ${event.icon}`, event.name, 'Permanent world trait');
    }
  }

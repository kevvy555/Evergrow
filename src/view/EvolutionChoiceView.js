...
export class EvolutionChoiceView extends Container {
  ...
  #render() {
    ...
    this.choice.options.forEach((option, index) => {
      const cardY = y + 85 + index * 112;
      const card = new Container(); card.position.set(x + 16, cardY);
      ...
      const desc = this.#text(option.description, 11, 600, 0xc9d8e8); desc.position.set(58, 43); desc.style.wordWrap = true; desc.style.wordWrapWidth = panelWidth - 106;
      const choose = this.#button('CHOOSE', panelWidth - 124, 12, () => this.onChoose(option.id));

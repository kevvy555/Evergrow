export class FeedbackService {
  ...
  handle(events) {
    ...
    else if (merges.length > 0) { const chain = Math.max(...merges.map((event) => event.chain)); this.#tone(300 + chain * 90, 0.045, 0.04); this.#vibrate(Math.min(24, 8 + chain * 4)); }
    else if (events.some((event) => event.type === 'spawn')) this.#tone(210, 0.018, 0.025);
  }
  #getAudioContext() { if (typeof window === 'undefined') return null; const AudioContext = window.AudioContext || window.webkitAudioContext; if (!AudioContext) return null; if (!this.audioContext) this.audioContext = new AudioContext(); if (this.audioContext.state === 'suspended') this.audioContext.resume().catch(() => {}); return this.audioContext; }
  #tone(frequency, gainValue, duration, delay = 0) { const context = this.#getAudioContext(); if (!context) return; const oscillator = context.createOscillator(); const gain = context.createGain(); const start = context.currentTime + delay; oscillator.type = 'sine'; oscillator.frequency.setValueAtTime(frequency, start); gain.gain.setValueAtTime(0.0001, start); gain.gain.exponentialRampToValueAtTime(gainValue, start + 0.01); gain.gain.exponentialRampToValueAtTime(0.0001, start + duration); oscillator.connect(gain); gain.connect(context.destination); oscillator.start(start); oscillator.stop(start + duration + 0.01); }
